...
var miner = {
	parts: [
		...
		[Game.MOVE, Game.MOVE, Game.WORK, Game.WORK, Game.WORK, Game.WORK, Game.WORK]
	],
	...
	setSourceToMine: function(source)
	{
		...
		var creepsNeeded = Math.ceil((steps * 10) / 100);
	...
	action: function()
	{
		...
		if(source === null) {
			...
			this.setSourceToMine(source);
			// console.log('ERROR - should already have source set');
			...
			creep.moveTo(source);
			var energy = creep.pos.findInRange(Game.DROPPED_ENERGY, 0)[0];
			if (!energy || energy.energy < 50)
